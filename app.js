...
document.getElementById("betForm").addEventListener("submit", (e) => {
    ...
    partido: `${local} vs ${visita}`,
         ...
         tipo: tipoJugada,
           ...
           // ⭐ NUEVO: NECESARIO PARA QUE EL LIKE FUNCIONE
         likes: 0,
         dislikes: 0
     };
...
function mostrarTabla() {
    ...
    jugadas.forEach((j, index) => {
        ...
        fila.innerHTML = `
            <td>${j.fecha}</td>
            <td>${j.hora || "--:--"}</td>                     <!-- NUEVO -->
            <td>${j.competicion || "Sin competencia"}</td>    <!-- NUEVO -->
...
document.getElementById("guardarCambiosBtn").addEventListener("click", () => {
    ...
    jugada.pronosticos.forEach((p, i) => {
        const nuevoEstado = document.getElementById(`editPronostico_${i}`).value;
        p.resultado = nuevoEstado;
    });

    if (jugada.pronosticos.every(p => p.resultado === "ganado")) {
        jugada.resultadoFinal = "ganado";
    } else if (jugada.pronosticos.some(p => p.resultado === "perdido")) {
        jugada.resultadoFinal = "perdido";
    } else {
        jugada.resultadoFinal = "pendiente";
    }
...
// DESBLOQUEAR JUGADA PREMIUM — POPUP YAPE
...
document.getElementById("confirmarPagoBtn").addEventListener("click", () => {
    ...
    jugada.tipo = "free"; // Se desbloquea
